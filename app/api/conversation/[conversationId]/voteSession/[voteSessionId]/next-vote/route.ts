...
export async function PATCH(request: Request, { params }: { params: Promise<{ voteSessionId: string }> }) {
    try {
        ...
        if (voteSession.status === "BEFORE_VOTING") {
            ...
            await pusherServer.trigger(voteSessionId, "voteSession_status:update", updatedVoteSession)
            ...
            const mostVotedIndex = trueCounts.map((count, index) => ({ count, index })).filter(item => item.count === maxVotes).map(item => item.index)

            const helper = mostVotedIndex.filter(item => !uniqueSuperDList.includes(item))
            ...
            if (helper.length !== 1) {
                ...
                await pusherServer.trigger(voteSessionId, "voteSession_round_one:end",updatedVoteSession)
            } else {
            ...
            await pusherServer.trigger(voteSessionId, "voteSession_round_two:end", winnerIndex)
