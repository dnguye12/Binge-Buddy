...
const MessageBox = ({ isLast, data }: MessageBoxProps) => {
  ...
  const message = cn(
    ...
    data.image
      ? "rounded-md p-0 bg-transparent drop-shadow-md"
        ...
        <div className={message}>
          {data.image ? (
            <Image
              ...
              width={144}
              src={data.image}
              className="object-hover translate cursor-pointer transition hover:scale-110"
              onClick={() =>
                onOpen("image", { image: data.image || undefined })
              }
            />
          ) : (
            <div>{data.body}</div>
