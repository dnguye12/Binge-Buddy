...
const VoteBox = ({ isLast, message }: VoteBoxProps) => {
  ...
  const handleGoToVote = () => {
    router.push(
      `/conversation/${message.conversationId}/voteSession/${message.voteSessionId}`,
    );
  };
  ...
  return (
    <div className={container}>
      ...
      <div className={body}>
        ...
        {message.voteSession?.status === "INITIATED" ||
        message.voteSession?.status === "BEFORE_VOTING" ? (
          <button
            onClick={() => {
              handleGoToVote();
            }}
          ...
          >
            {isOwn ? (
              <>
                <VoteIcon className="h-auto w-9" />
                <div className="flex flex-col items-start justify-center">
                  <p className="text-base leading-none font-semibold tracking-tight">
                    You started a vote
                  </p>
                  <p className="text-primary-foreground mt-1 text-center text-sm">
              ...
              <>
                ...
                <div className="flex flex-col items-start justify-center">
                  <p className="text-base leading-none font-semibold tracking-tight">
                    {message.sender.name} started a vote
                  </p>
                  <p className="text-secondary-foreground mt-1 text-center text-sm">
                    Join the session now.
                  </p>
                </div>
              </>
          ...
          <button
            ...
            className={messageClass}
          >
            <TvMinimalPlayIcon className="h-auto w-9" />
            <div className="flex flex-col items-start justify-center">
              <p className="text-base leading-none font-semibold tracking-tight">
                Movie is playing.
              </p>
        ...
        ) : message.voteSession?.status === "ROUND_ONE" ||
          message.voteSession?.status === "ROUND_TWO" ? (
          <button
            onClick={() => {
              handleGoToVote();
            }}
          ...
          <button disabled={true} className={messageClass}>
            <CircleXIcon className="h-auto w-9" />
            <div className="flex flex-col items-start justify-center">
              <p className="text-base leading-none font-semibold tracking-tight">
                Vote link expired
              </p>
